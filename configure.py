...
def download_depends():
	...
	if not os.path.exists(xerces_path):
		...
		os.system(f'curl http://archive.apache.org/dist/xerces/c/3/sources/xerces-c-{xerces_version}.tar.gz > xerces.tar.gz')
	...
	if not os.path.exists(cspice_path):
		...
		if sys.platform == 'win32':
			...
			os.system('curl -L http://naif.jpl.nasa.gov/pub/naif/misc/toolkit_' + cspice_version + '/C/PC_Windows_VisualC_' + cspice_bit + '/packages/cspice.zip > cspice.zip')
	...
	if not os.path.exists(swig_dir):
		...
		if sys.platform == 'win32':
			...
			os.system('curl -L -k http://download.sourceforge.net/swig/swigwin-' + swig_version + '.zip > swig.zip')  # my code: added -k
...
def build_xerces():
	...
	if sys.platform == 'win32':
		...
		if not os.path.exists(xerces_outdir):
			...
			os.system(f'cmake -G "Visual Studio {vs_major_version} {str(vs_version)}" -DBUILD_SHARED_LIBS:BOOL=OFF -Dtranscoder=windows -DCMAKE_INSTALL_PREFIX="{xerces_outdir}" "{xerces_path}" > "{logs_path}\\xerces_cmake.log" 2>&1')
			...
			os.system(f'cmake --build . --config Release --target install > "{logs_path}\\xerces_build_release.log" 2>&1')
		else:
	...
	if sys.platform == 'win32':
		...
		if not os.path.exists(cspice_path + '/' + cspice_dir + '/lib/cspiced.lib'):
			...
			def compile_cspice(build_type):
				...
				os.system(f'cl /c /DEBUG /Z7 /MP -D_COMPLEX_DEFINED -DMSDOS -DOMIT_BLANK_CC -DNON_ANSI_STDIO -DUIOLEN_int *.c > "{logs_path}\\cspice_build_{build_type}.log" 2>&1')
				os.system(f'link -lib /out:..\\..\\lib\\cspiced.lib *.obj >> "{logs_path}\\cspice_build_{build_type}.log" 2>&1')
